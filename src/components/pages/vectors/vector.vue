<template>
  <div>
    <div id="box1" class="jsx-graph my-2"></div>
    <div id="box2" class="jsx-graph my-2"></div>
  </div>
</template>
...
export default {
  mounted() {
    ...
    var w2 = b2.create(
      ...
      [
        function() {
          return a.Value() * w.X() + b.Value() * w.Y();
        },
        function() {
          return c.Value() * w.X() + d.Value() * w.Y();
        }
      ],
      { face: "[]", size: 2, name: "w'" }
    );
    ...
    var t = b2.create("text", [
      -6,
      -2,
      function() {
        return "|" + a.Value().toFixed(2) + " " + b.Value().toFixed(2) + "|" + "<br>" + "|" + c.Value().toFixed(2) + " " + d.Value().toFixed(2) + "|";
      }
    ]);
    b2.unsuspendUpdate();
  }
